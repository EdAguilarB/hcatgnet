...
def plot_denoised_mols(
    ...
    importances.append(
        mask[:, atom_identity + degree + hyb : atom_identity + degree + hyb + aromatic]
    )
    importances.append(
        mask[
            :,
    ...
    plot_weighted_mol(importance, smiles, norm)


def plot_weighted_mol(mask, smiles: str, norm=False):
...
def visualize_score_features(
    ...
    if mol:
        ...
        importances.append(
            score[ia:fa, atom_identity + degree : atom_identity + degree + hyb]
        )
        importances.append(
            score[
                ia:fa,
                atom_identity + degree + hyb : atom_identity + degree + hyb + aromatic,
            ]
        )
        importances.append(
            score[
        ...
        importances.append(
            score[:, atom_identity + degree : atom_identity + degree + hyb]
        )
        importances.append(
            score[
                :,
...
sizes = {"C": 69 / 2, "O": 66 / 2, "N": 71 / 2, "H": 31 / 2, "B": 84 / 2, "F": 64 / 2}
