...
class hypervalent_graph(reaction_graph):
    ...
    def process(self):
        ...
        for index, reaction in tqdm(self.data.iterrows(), total=self.data.shape[0]):
            ...
            temp_tensor = torch.full((node_feats_reaction.shape[0], 1), temp)
            conc_tensor = torch.full((node_feats_reaction.shape[0], 1), conc)
            cpba_tensor = torch.full((node_feats_reaction.shape[0], 1), cpba)

            node_feats_reaction = torch.cat([node_feats_reaction, temp_tensor, conc_tensor, cpba_tensor], axis=1)
