...
class hydrogenation_reaction_graph(reaction_graph):
    ...
    def __init__(
        self, opt: argparse.Namespace, filename: str, molcols: list, root: str = None
    ) -> None:
        ...
        if self._include_fold:
            if os.path.exists(
                os.path.join(root, "raw", f"{filename[:-4]}_folds{filename[-4:]}")
            ):
                file_folds = filename[:-4] + "_folds" + filename[-4:]
                self.folds = pd.read_csv(os.path.join(root, "raw", f"{file_folds}"))[
                    "fold"
                ]
            else:
                self.folds = self.split_data(
                    root, filename, opt.folds, opt.global_seed, opt
                )

            filename = filename[:-4] + "_folds" + filename[-4:]

        else:
            self.folds = pd.read_csv(os.path.join(root, "raw", f"{filename}"))[
                opt.splits_col
            ]

        self.targets = torch.tensor(
            pd.read_csv(os.path.join(root, "raw", f"{filename}"))[opt.target_col],
            dtype=torch.float,
        )
        self.id = pd.read_csv(os.path.join(root, "raw", f"{filename}"))[
            "limsID"
        ].astype(str)
        ...
        for index, reaction in tqdm(self.data.iterrows(), total=self.data.shape[0]):
            ...
            for reactant in self.mol_cols:
                ...
                node_feats = torch.cat(
                    [node_feats, temp_feat, preasure_feat, sc_feat], axis=1
                )
                ...
                torch.save(
                    mol_graph,
                    os.path.join(self.processed_dir, f"{reactant}_{index}.pt"),
                )
    ...
    def _create_empty_graph(self):
        return Data(
            x=torch.tensor([0 for _ in range(49)]).unsqueeze(0),
            edge_index=torch.tensor([[0], [0]]),
            edge_attr=torch.tensor([0 for _ in range(9)]).unsqueeze(0),
            smiles="",
        )
    ...
    def get(self, idx):
        ...
        for mol in self.mol_cols:
            data[mol] = torch.load(
                os.path.join(self.processed_dir, f"{mol}_{idx}.pt"), weights_only=False
            )
    ...
    def split_data(self, root, filename, n_folds, random_seed, opt):
        ...
        dataset["fold"] = dataset.index.map(index_dict) + 1
