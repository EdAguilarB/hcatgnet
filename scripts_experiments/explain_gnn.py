...
def denoise_graphs(opt, exp_path: str) -> None:
    ...
    train_loader = torch.load(
        os.path.join(model_path, "train_loader.pth"), weights_only=False
    )
    val_loader = torch.load(
        os.path.join(model_path, "val_loader.pth"), weights_only=False
    )
    test_loader = torch.load(
        os.path.join(model_path, "test_loader.pth"), weights_only=False
    )
    ...
    model_params = torch.load(
        os.path.join(model_path, "model_params.pth"), weights_only=True
    )
...
def GNNExplainer_node_feats(opt, exp_path: str) -> None:
    ...
    ligand_masks, substrate_masks, boron_masks, _ = explain_dataset(
        test_loader.dataset, explainer
    )
