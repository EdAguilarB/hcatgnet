...
def plot_results(
    save_dir,
    experiments_gnn_path,
    experiments_tml_path,
    folds=10,
    tml_algorithm="rf",
):
    ...
    for outer in range(1, folds + 1):
        ...
        outer_gnn = os.path.join(experiments_gnn_path, f"Fold_{outer}_test_set")
        outer_tml = os.path.join(experiments_tml_path, f"Fold_{outer}_test_set")

        metrics_gnn = extract_metrics(
            file=outer_gnn + f"/performance_outer_test_fold{outer}.txt"
        )
        metrics_tml = extract_metrics(
            file=outer_tml + f"/performance_outer_test_fold{outer}.txt"
        )
    ...
    create_parity_plot(
        data=results_all, save_path=save_dir, tml_algorithm=tml_algorithm
    )
    ...
    tml_predictions = results_all[results_all["Method"] == tml_algorithm]
    print(f"Final metrics for {tml_algorithm}:")
    print(
        "Accuracy: {:.3f}".format(
            accuracy_score(
